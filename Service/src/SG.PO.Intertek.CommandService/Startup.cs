...
namespace SG.PO.Intertek.CommandService
{
        ...
        public void ConfigureServices(IServiceCollection services)
        {
          ...
          v => LogContext.Push(v.Select(x => (ILogEventEnricher)new PropertyEnricher(x.Key, x.Value)).ToArray())
            ...
            services.AddMvc(options =>
            {
                ...
                options.Filters.Add(typeof(ElasticIndexFilter<POIntertekOutput>));
            });
            services.AddLogging();
            services.AddMvcCore().AddJsonFormatters();
            services.AddTransient<POIntertekService>();
            ...
            services.AddElastic<POIntertekOutput>(options =>
            {
                options.NodeList = settings.GetSection("NodeList").Get<string[]>();
                options.UserName = settings["UserName"];
                options.Password = settings["Password"];
            }, p => p.PONumber);  // To be changed to PO NUmber
        }
