...
namespace SG.PO.Chino.Ingester
{
    public class SGIngester : SG.Ingester.Ingester
    {
        ...
        {
            Console.Title = "SG.PO.Chino.Ingester";
        ...
        private IHandle<Message> CreateHttpHandler()
        {
            ...
            //PO
            handlers.Add(POEvents.MessageTypes.For<POEvents.MMSPOCreatedEvent>(), HttpInvokingHandler.CreateHandler<POEvents.MMSPOCreatedEvent>(HttpMethod.Post, msg => new Uri($"poChino/cmd/create", UriKind.Relative), HttpStatusCode.NoContent));

            handlers.Add(POEvents.MessageTypes.For<POEvents.MMSPOUpdatedEvent>(), HttpInvokingHandler.CreateHandler<POEvents.MMSPOUpdatedEvent>(HttpMethod.Post, msg => new Uri($"poChino/cmd/update", UriKind.Relative), HttpStatusCode.NoContent));

            //POsku
            handlers.Add(POEvents.MessageTypes.For<POEvents.MMSPOSkuCreatedEvent>(), HttpInvokingHandler.CreateHandler<POEvents.MMSPOSkuCreatedEvent>(HttpMethod.Post, msg => new Uri($"poChino/cmd/posku/create", UriKind.Relative), HttpStatusCode.NoContent));

            handlers.Add(POEvents.MessageTypes.For<POEvents.MMSPOSkuUpdatedEvent>(), HttpInvokingHandler.CreateHandler<POEvents.MMSPOSkuUpdatedEvent>(HttpMethod.Post, msg => new Uri($"poChino/cmd/posku/update", UriKind.Relative), HttpStatusCode.NoContent));
